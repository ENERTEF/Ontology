...
def get_simSN(lst1, lst2):
    ...
    for word_a in lst1:
        for word_b in lst2:
            if Word_net(word_a, word_b).path_sim() >= 0.8:
                simSN.append(word_a)
...
def get_SSG(lst):
    ...
    for word in lst:
        if wn.synsets(word):
            polys.append(len(wn.synsets(word)))
        else:
            lst.remove(word)
...
def get_SG(list_a, list_b):
    ...
    for i in range(len(list_a)):
        min_length = list_a[i].shortest_path_distance(list_b[i])
        if min_length is not None:
            length = min_length
            for hyper in list_a[i].lowest_common_hypernyms(list_b[i]):
...
if __name__ == '__main__':
    ...
    # Extend the word chain into themselves and their synonyms and hyponyms
    word_chain_new = get_Synonymsandhyponyms(word_chain)
    print('The synonyms and hyponyms of the word chain:', word_chain_new)

    gg_list = [get_GG("Ontologies/saref4ener.ttl", word_chain_new),
    ...
    #            get_GG("Ontologies/respond.owl", word_chain_new),
